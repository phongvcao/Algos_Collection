...
public:
    ...
    void loadData() {
        ...
        // lock_guard will release the lock when this function ends.
        lock_guard< mutex > guard( mut );

        // Set isDataLoaded to true
        isDataLoaded = true;

        // Notify the conditional variable so that it would lock the mutex again
        // and check for the conditional variable.
        //
        // This should always be at the end of the function so right after
        // condVar notifies other threads, lock_guard would be destroyed.
        condVar.notify_one();
    }
    ...
    void mainTask() {
        ...
        // Acquire the lock. This thread owns the mutex
        unique_lock< mutex > uLock( mut );

        // Now since unique_lock can be locked/unlocked at will, condVar.wait()
        // will first unlock the lock & wait for signal from loadData() to
        // lock the lock again. When it locks the unique_lock it will invoke
        // the function isDataLoaded. If isDataLoaded() is true, it proceeds
        // to the next line. Otherwise, it keeps waiting.

        // condVar.wait( uLock, bind( &Application::isDataLoaded, this ) );
        condVar.wait( uLock, [ this ]{ return isDataLoaded; } );

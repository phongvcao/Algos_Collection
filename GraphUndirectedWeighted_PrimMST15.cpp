...
public:
    ...
    friend ostream& operator <<( ostream& out, const MinPQ< Key >& minPQ ) {
        out << "\npq:\n";
...
public:
    ...
    LazyPrimMSTMinPQ( const EdgeWeightedGraph& G )
            : marked( G.getV(), false ),
              minPQ( G.getV() )
    {
        visit( G, 0 );
    ...
    friend ostream& operator <<( ostream& out, const LazyPrimMSTMinPQ& mst ) {
        out << "\nmst:\n";
        queue< Edge > q = mst.getEdges();
        while ( ! q.empty() ) {
            out << q.front() << ", ";
...
int main( int argc, char ** argv ) {
    ...
    LazyPrimMST mst1MinPQ( ewGraph );
    cout << "DEBUG: LazyPrimMSTMinPQ: \n" << mst1MinPQ << endl;
    cout << "DEBUG: LazyPrimMSTMinPQ.getWeight(): \n" << mst1MinPQ.getWeight() << endl;

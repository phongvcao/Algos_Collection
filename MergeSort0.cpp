...
private:
    ...
    static void merge( vector< Value >& orgVec, vector< Value >& auxVec, int low, int mid, int high ) {
        // precondition: orgVec[ lo .. mid ] and orgVec[ mid+1 .. hi ] are sorted subarrays
        // copy to auxVec
    ...
    static void sort( vector< Value >& orgVec, vector< Value >& auxVec, int low, int high ) {
        if ( high <= low ) return;
...
public:
    ...
    static void sort( vector< Value >& orgVec ) {
        vector< Value > auxVec( orgVec.size() );
...
int main( int argc, char ** argv ) {
    ...
    Merge< int >::sort( intArr );
    cout << "DEBUG : Merge< int >::sort() : " << endl;

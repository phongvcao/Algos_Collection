...
//
class LazyDijkstraConciseKStopSP {
private:
    ...
    public:
        ...
        Path( int to, double weight, int k )
            : to( to ),
              weight( weight ),
              k( k )
        {
    ...
    priority_queue< Path, vector< Path >, cmpPaths > minPQ;
    vector< double > distTo;
    vector< pair< int, int > > edgeTo;
    const vector< unordered_map< int, double > > * graph;

    void relax( int v, double weight, int k ) {
        for ( auto iter = graph->at( v ).begin(); iter != graph->at( v ).end(); iter++ ) {
            int w = iter->first;
            double curEdgeWeight = iter->second;
            distTo[ w ] = weight + curEdgeWeight;
            edgeTo[ w ] = { v, w };
            minPQ.emplace( w, weight + curEdgeWeight, k + 1 );
        }
    }

public:

    LazyDijkstraConciseKStopSP( const vector< unordered_map< int, double > >& G,
                                int s, int t, int K )
        : distTo( G.size(), numeric_limits< double >::infinity() ),
          edgeTo( G.size(), { -1, -1 } ),
          graph( &G )
    {
        minPQ.emplace( s, 0, 0 );
        while ( ! minPQ.empty() ) {
            ...
            relax( w, p.weight, p.k );
        }
    }

    virtual ~LazyDijkstraConciseKStopSP() {
        // empty for now
    }
    ...
    pair< int, int > getEdgeTo( int v ) const {
        return edgeTo[ v ];
    }

    stack< pair< int, int > > getPathTo( int v ) const {
        stack< pair< int, int > > pathStack;
        if ( ! hasPathTo( v ) ) return pathStack;
        for ( pair< int, int > e = edgeTo[ v ]; e.first != -1; e = edgeTo[ e.first ] ) {
            pathStack.push( e );
        }
        return pathStack;
    }
    ...
    friend ostream& operator <<( ostream& out, const LazyDijkstraConciseKStopSP& sp ) {
        out << "\nsp:\n";
        for ( int v = 0; v < sp.edgeTo.size(); v++ ) {
            out << "\n| v : " << v << " : ";
            stack< pair< int, int > > s = sp.getPathTo( v );
            while ( ! s.empty() ) {
                int v = s.top().first;
                int w = s.top().second;
                double weight = sp.graph->at( v ).at( w );
                out << " | " << v << " : " << w << " : " << weight << " | , ";
                s.pop();
            }
            out << " | ";
        ...
        out << "\n\nedgeTo:\n";
        for ( int v = 0; v < sp.edgeTo.size(); v++ ) {
            int from = sp.edgeTo[ v ].first;
            int to = sp.edgeTo[ v ].second;
            if ( from != -1
                    && to != -1
                    && sp.graph->at( from ).find( to ) != sp.graph->at( from ).end() ) {
                double weight = sp.graph->at( from ).at( to );
                out << v << " :  | " << from << " : " << to << " : " << weight << " | " << endl;
            } else {
                double weight = numeric_limits< double >::infinity();
                out << v << " :  | " << from << " : " << to << " : " << weight << " | " << endl;
            }
...
int main( int argc, char ** argv ) {
    ...
    vector< unordered_map< int, double > > ewDigraph( 8,
                                                      unordered_map< int, double >() );
    for ( int i = 0; i < 15; ++i ) {
        ewDigraph[ graphArr[ i ][ 0 ] ].insert( { graphArr[ i ][ 1 ],
                                                  graphArr[ i ][ 2 ] } );
    }
    ...
    for ( int v = 0; v < 8; ++v ) {
        cout << "\n| v : " << v << " : ";
        for ( auto iter = ewDigraph[ v ].begin(); iter != ewDigraph[ v ].end(); ++iter ) {
            int w = iter->first;
    ...
    cout << endl;
    LazyDijkstraConciseKStopSP sp( ewDigraph, 0, 7, 7 );
    cout << "DEBUG: sp: \n" << sp << endl;

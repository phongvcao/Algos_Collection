...
//
// Time Cpmlexity: O( N ^ 2 )
// Space Complexity: O( N ^ 2 )
//
class MultipleSameSizeArrays {
public:
    unordered_map< int, int > countAllSumCombinations( const vector< vector< int > >& vectorsList, int start, int end ) const {
        unordered_map< int, int > count( { { 0, 1 } } );

        for ( int i = start; i < end; i++ ) {
            unordered_map< int, int > tempCount;
            for ( int value : vectorsList[ i ] ) {
                for ( auto countIter = count.begin(); countIter != count.end(); ++countIter ) {
                    int sumSoFar = countIter->first;
                    int sumSoFarCount = countIter->second;
                    tempCount[ sumSoFar + value ] += sumSoFarCount;
                }
            }
            count = tempCount;
        }
    ...
    unordered_map< int, int > countAllProductCombinations( const vector< vector< int > >& vectorsList, int start, int end ) const {
        unordered_map< int, int > count( { { 1, 1 } } );
        for ( int i = start; i < end; ++i ) {
            unordered_map< int, int > tempCount;
            for ( int value : vectorsList[ i ] ) {
                for ( auto countIter = count.begin(); countIter != count.end(); ++countIter ) {
                    int productSoFar = countIter->first;
                    int productSoFarCount = countIter->second;
                    tempCount[ productSoFar * value ] += productSoFarCount;
                }
            }
            count = tempCount;
        }
    ...
    static string toString( const unordered_map< int, int >& count ) {
        string rv = "";
        for ( auto countIter = count.begin(); countIter != count.end(); ++countIter ) {
            if ( countIter != count.begin() ) {
                rv += ", ";
            }
            rv += "{ " + to_string( countIter->first ) + ", " + to_string( countIter->second ) + " }";
        }
        return " { " + rv + " }";
    }
};
...
int main( int argc, char ** argv ) {
    ...
    MultipleSameSizeArrays mssa;

    vector< vector< int > > input = { { 1, 2 }, { -2, -1 }, { -1, 2 }, { 0, 2 } };
    unordered_map< int, int > result = mssa.countAllSumCombinations( input, 0, input.size() );
    cout << "Case 1 - countAllSumCombinations():" << endl;
    cout << "Want: { { 4, 2 }, { -1, 2 }, { 0, 2 }, { 3, 2 }, { -2, 1 }, { 2, 3 }, { 1, 3 }, { 5, 1 } } " << endl;
    cout << "Got: " << MultipleSameSizeArrays::toString( result ) << endl;

    cout << endl;

    input = { { 0 }, { 0 }, { 0 }, { 0 } };
    result = mssa.countAllSumCombinations( input, 0, input.size() );
    cout << "Case 2 - countAllSumCombinations():" << endl;
    cout << "Want: { { 0, 1 } } " << endl;
    cout << "Got: " << MultipleSameSizeArrays::toString( result ) << endl;
    ...
    result = mssa.countAllProductCombinations( input, 0, input.size() );
    cout << "Case 1 - countAllProductCombinations():" << endl;
    cout << "Want: { { -16, 1 }, { 0, 8 }, { 4, 2 }, { 2, 1 }, { 8, 1 }, { -8, 2 }, { -4, 1 } } " << endl;
    cout << "Got: " << MultipleSameSizeArrays::toString( result ) << endl;
    ...
    input = { { 0 }, { 0 }, { 0 }, { 0 } };
    result = mssa.countAllProductCombinations( input, 0, input.size() );
    cout << "Case 2 - countAllProductCombinations():" << endl;
    cout << "Want: { { 0, 1 } } " << endl;
    cout << "Got: " << MultipleSameSizeArrays::toString( result ) << endl;

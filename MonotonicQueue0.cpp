...
public:
    ...
    // Increasing ( or Non-Decreasing ) from front() to back() ( push()'s order )
    // Keep pop()-ing BACK elements until val >= innerDeque.back()
    static const int MONO_INCREASING;
    // Decreasing ( or Non-Increasing ) from front() to back() ( push()'s order )
    // Keep pop()-ing BACK elements until val <= innerDeque.back()
    static const int MONO_DECREASING;

    MonoQueue( int monoType )
        : monoType( monoType )
    {
        // empty for now
    }
    ...
    void push( Value val ) {
        while ( ! innerDeque.empty() && ! isValid( val ) ) {
            innerDeque.pop_back();
        }
        innerDeque.push_back( val );
    }
    ...
    Value front() const {
        return innerDeque.front();
    }

    Value back() const {
        return innerDeque.back();
    }
...
template< typename Value >
const int MonoQueue< Value >::MONO_INCREASING = -1;
template< typename Value >
const int MonoQueue< Value >::MONO_DECREASING = 1;

int main( int argc, char ** argv ) {

    vector< int > intArr = { 14, 12, 10, 8, 6, 4, 2, 0, 1, 3, 5, 7, 9, 11, 13 };

    MonoQueue< int > q1( MonoQueue< int >::MONO_INCREASING );
    for ( int i = 0; i < intArr.size(); i++ ) {
        q1.push( intArr[ i ] );
    }
    cout << "DEBUG : Monotonic Increasing Queue : " << endl;
    while ( ! q1.empty() ) {
        cout << q1.front() << ", ";
        q1.pop();
    }
    ...
    MonoQueue< int > q2( MonoQueue< int >::MONO_DECREASING );
    for ( int i = 0; i < intArr.size(); i++ ) {
        q2.push( intArr[ i ] );
    }
    cout << "DEBUG : Monotonic Decreasing Queue : " << endl;
    while ( ! q2.empty() ) {
        cout << q2.front() << ", ";
        q2.pop();
    }

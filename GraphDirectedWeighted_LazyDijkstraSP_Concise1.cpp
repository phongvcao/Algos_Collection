...
public:
    ...
    LazyDijkstraConciseSP( const vector< unordered_map< int, double > >& G,
                           int s )
        : relaxed( G.size(), false ),
          ...
          graph( &G )
    {
        ...
        while ( ! minPQ.empty() ) {
            ...
            minPQ.pop();
            int w = p.to;
            if ( ! relaxed[ w ] ) relax( w );
        }
    }

    virtual ~LazyDijkstraConciseSP() {
        // empty for now
    }
    ...
    friend ostream& operator <<( ostream& out, const LazyDijkstraConciseSP& sp ) {
        out << "\nsp:\n";
        ...
        for ( int v = 0; v < sp.edgeTo.size(); v++ ) {
            ...
            if ( from != -1 && to != -1 && sp.graph->at( from ).find( to ) != sp.graph->at( from ).end() ) {
...
int main( int argc, char ** argv ) {
    ...
    for ( int i = 0; i < 15; ++i ) {
        ewDigraph[ graphArr[ i ][ 0 ] ].insert( { graphArr[ i ][ 1 ], graphArr[ i ][ 2 ] } );
    }
    ...
    cout << endl;
    LazyDijkstraConciseSP sp( ewDigraph, 0 );
    cout << "DEBUG: sp: \n" << sp << endl;

...
using std::timed_mutex;                     // mutex with a timeout
using std::recursive_mutex;                 // mutex can be locked recursively by the same thread
using std::recursive_timed_mutex;           // mutex can be locked recursively by same thread and locking with a timeout
using std::lock;                            // locks specified mutexes, blocks if any are unavailable
using std::lock_guard;                      // strictly scope-based mutex ownership wrapper
using std::scoped_lock;                     // deadlock-avoiding RAII wrapper for multiple mutexes
using std::unique_lock;                     // movable mutex ownership wrapper
...
public:
    ...
    void addMoney( int amount ) {
        // This automatically does lock-destruction-management for us
        // this this function's scope
        lock_guard< mutex > lockGuard( wMutex );

        for ( int i = 0; i < amount; i++ ) {
            // If some exception occurs at this point destructor of lockGuard
            // will be called due to stack unwinding.
            ++money;
        }
        
        cout << "DEBUG : thread : " << this_thread::get_id() << " ; money : " << money << endl;
        
        // Once function exists, lockGuard destructor will be called & it will
        // unlock the mutex
    }
...
int main( int argc, char ** argv ) {
    ...
    // Fire up the threads
    for ( int i = 0; i < 5; i++ ) {
        threadsVec.push_back( thread( &Wallet::addMoney, &walletObj, 1000 ) );
    }
    ...
    cout << "DEBUG : walletObj.getMoney() : " << walletObj.getMoney() << endl;

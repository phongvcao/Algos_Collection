...
public:
    static int indexOf( const vector< Value >& valuesVec, Value val ) {
        int low = 0;
        int high = valuesVec.size() - 1;

        while ( low <= high ) {
            int mid = low + ( high - low ) / 2;

            if ( val < valuesVec[ mid ] ) {
                //
                // We set high this way because low < high instead of low <= high
                //
                high = mid - 1;
            } else {
                //
                // Even after we find mid, we're still not satisfied and keep
                // going right by setting low = mid + 1 ( instead of simply
                // return mid ) to see if we can find a valuesVec[ mid ] that
                // has larger mid.
                //
                // At the end, low - 1 will either contain the largest mid, or
                // low contains the only found element or low not containing mid
                // at all. We have to check valuesVec[ low - 1 / low ] for that.
                //
                low = mid + 1;
            }
        }

        if ( low > 0 && valuesVec[ low - 1 ] == val ) return low - 1;
        if ( valuesVec[ low ] == val ) return low;
        return -1;
    }
};
...
int main( int argc, char ** argv ) {
    ...
    vector< int > intArr = { 0, 0, 1, 2, 3, 4, 5, 5, 5, 5, 6, 7, 8, 9, 10, 10, 10 };

    cout << "DEBUG : BinarySearchRightMost::indexOf( 0 ) == 1 : " << BinarySearchRightMost< int >::indexOf( intArr, 0 ) << endl;
    cout << "DEBUG : BinarySearchRightMost::indexOf( 5 ) == 9 : " << BinarySearchRightMost< int >::indexOf( intArr, 5 ) << endl;
    cout << "DEBUG : BinarySearchRightMost::indexOf( 10 ) == 16 : " << BinarySearchRightMost< int >::indexOf( intArr, 10 ) << endl;
    cout << "DEBUG : BinarySearchRightMost::indexOf( 4 ) == 5 : " << BinarySearchRightMost< int >::indexOf( intArr, 4 ) << endl;
    cout << "DEBUG : BinarySearchRightMost::indexOf( 50 ) == -1 : " << BinarySearchRightMost< int >::indexOf( intArr, 50 ) << endl;

...
enum MonoStackType {
    MONO_STACK_TYPE_INCREASING = 0,
    MONO_STACK_TYPE_DECREASING = 1,
};
...
private:
    ...
    MonoStackType type = MONO_STACK_TYPE_INCREASING;

    bool isValid( Value val ) const {
        if ( type == MONO_STACK_TYPE_INCREASING ) {
...
int main( int argc, char ** argv ) {
    ...
    MonoStack< int > s1( MONO_STACK_TYPE_INCREASING );
    ...
    MonoStack< int > s2( MONO_STACK_TYPE_DECREASING );
    for ( int i = 0; i < intArr.size(); i++ ) {
        s2.push( intArr[ i ] );
    }

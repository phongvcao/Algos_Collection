...
public:
    ...
    static int indexOf( const vector< Value >& valuesVec, Value val ) {
        if ( valuesVec[ 0 ] == val ) return 0;

        int i = 1;
        while ( i < valuesVec.size() && val > valuesVec[ i ] ) {
            i *= 2;
        }

        int low = i / 2;
        int high = min( i, ( int )valuesVec.size() );
...
int main( int argc, char ** argv ) {
    ...
    cout << "DEBUG : ExponentialSearch::indexOf( 5 ) : " << ExponentialSearch< int >::indexOf( intArr, 5 ) << endl;
    cout << "DEBUG : ExponentialSearch::indexOf( 50 ) : " << ExponentialSearch< int >::indexOf( intArr, 50 ) << endl;

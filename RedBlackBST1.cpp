...
private:
    ...
    public:
        ...
        int N = 0;
        bool color = RED;
        // Index of unique_ptr< Node > in nodeVec
        int index = -1;
    ...
    // To delete a Node from nodeVec, we swap the Node to be deleted with the last node in the vector,
    ...
    void freeNode( Node * h ) {
        if ( h->index < nodeVec.size() - 1 ) {
            nodeVec[ h->index ].swap( nodeVec[ nodeVec.size() - 1 ] );
            nodeVec[ h->index ]->index = h->index;
        }
        nodeVec.pop_back();
    }

    Node * newNode( Key key, Value value, int N, bool color ) {
        nodeVec.push_back( make_unique< Node >( key, value, N, color ) );
        nodeVec.back()->index = nodeVec.size() - 1;
        return nodeVec.back().get();
    }
...
public:
    ...
    void printNodeVec() const {
        for ( const unique_ptr< Node >& ptr : nodeVec ) {
...
int main( int argc, char ** argv ) {
    ...
    cout << "DEBUG: bst.nodeVec.size(): " << bst.nodeVec.size() << endl;

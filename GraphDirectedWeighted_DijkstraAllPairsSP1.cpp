...
private:
    ...
    vector< unique_ptr< DijkstraSP > > all;
...
public:
    ...
    {
        for ( int v = 0; v < G.getV(); ++v ) {
            all[ v ] = make_unique< DijkstraSP >( G, v );
        }
    }

    virtual ~DijkstraAllPairsSP() {
        // not implemented
    }
    ...
    friend ostream& operator <<( ostream& out, const DijkstraAllPairsSP& sp ) {
        out << "\nsp:\n";
        for ( int v = 0; v < sp.all.size(); ++v ) {
            out << "\n| v : " << v << " : ";
            for ( int w = 0; w < sp.all.size(); ++w ) {
                out << "\n| w : " << w << " : ";
...
int main( int argc, char ** argv ) {
    ...
    DijkstraAllPairsSP sp( ewDigraph );
    cout << "DEBUG: sp: \n" << sp << endl;

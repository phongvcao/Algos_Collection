...
public:
    ...
    TopologicalConcise( const vector< vector< int > >& G )
        : graph( &G ),
          inDegrees( G.size(), 0 ),
          order( G.size(), 0 )
    {
...
int main( int argc, char ** argv ) {
    ...
    vector< vector< int > > dGraph( 13, vector< int >() );

    for ( int i = 0; i < 16; ++i ) {
        dGraph[ graphArr[ i ][ 0 ] ].push_back( graphArr[ i ][ 1 ] );
    }
    ...
    TopologicalConcise topo( dGraph );
    cout << "DEBUG: topo: \n" << topo.toString() << endl;

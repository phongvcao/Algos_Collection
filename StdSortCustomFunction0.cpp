...
public:
    ...
    void sort( vector< Value >& a, bool useCompStruct = true ) {
        if ( useCompStruct ) {
            std::sort( a.begin(), a.end(), customLessClass() );
        } else {
            std::sort( a.begin(), a.end(), []( const Value& a, const Value& b ) {
                return a < b;
            });
        }
    }
...
int main( int argc, char ** argv ) {
    ...
    CustomSort< int > customSort;
    customSort.sort( intVec, true );
    ...
    intVec = { 14, 12, 10, 8, 6, 4, 2, 0, 1, 3, 5, 7, 9, 11, 13 };
    customSort.sort( intVec, false );

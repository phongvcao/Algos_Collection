...
//
// Hamiltonian Cycles / Paths are cycles / paths that go through all vertices
...
private:
    ...
    vector< vector< int > > allCyclesVec;

    void dfs( int v, vector< bool > marked, vector< int > pathVec ) {
        if ( pathVec.size() == graph->size() ) {
            if ( graph->at( pathVec.back() ).find( pathVec[ 0 ] ) != graph->at( pathVec.back() ).end() ) {
                //
                // Find cycles
                //
                pathVec.push_back( pathVec[ 0 ] );
                allCyclesVec.push_back( pathVec );
            } else {
                //
                // Find paths
                //
                allPathsVec.push_back( pathVec );
            }
            return;
        }
...
public:
    ...
    Hamiltonian( const vector< unordered_set< int > >& G )
        : graph( &G )
    {
    ...
    vector< vector< int > > getAllPaths() const {
        return allPathsVec;
    }
    ...
    bool hasHamiltonianPath() const {
        return ! allPathsVec.empty();
    }

    bool hasHamiltonianCycle() const {
        return ! allCyclesVec.empty();
    }

    friend ostream& operator <<( ostream& out, const Hamiltonian& hamilton ) {
        out << "\nhamiltonian paths:\n";
        for ( int i = 0; i < hamilton.allPathsVec.size(); i++ ) {
            for ( int j = 0; j < hamilton.allPathsVec[ i ].size(); j++ ) {
                out << hamilton.allPathsVec[ i ][ j ] << " , ";
            }
            out << endl;
        }
        out << "\nhamiltonian cycles:\n";
        for ( int i = 0; i < hamilton.allCyclesVec.size(); i++ ) {
            for ( int j = 0; j < hamilton.allCyclesVec[ i ].size(); j++ ) {
                out << hamilton.allCyclesVec[ i ][ j ] << " , ";
            }
            out << endl;
        }
        return out;

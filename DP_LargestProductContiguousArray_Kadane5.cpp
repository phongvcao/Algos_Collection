...
//
class LargestProductKadane {
public:

    int getLargestProductContiguousArray( const vector< int >& intVec ) {
        if ( intVec.empty() ) return numeric_limits< int >::lowest();

        int maxSoFar = intVec[ 0 ];
        int minSoFar = intVec[ 0 ];
        int result = maxSoFar;

        for ( int i = 1; i < intVec.size(); i++ ) {
            int tempMax = max( intVec[ i ], max( maxSoFar * intVec[ i ], minSoFar * intVec[ i ] ) );
            minSoFar = min( intVec[ i ], min( maxSoFar * intVec[ i ], minSoFar * intVec[ i ] ) );
            maxSoFar = tempMax;
...
int main( int argc, char ** argv ) {
    ...
    LargestProductKadane kadane;

    vector< int > intVec = { 2, 3, -2, 4 };
    cout << "DEBUG : kadane.getLargestProductContiguousArray() == 6 : " << kadane.getLargestProductContiguousArray( intVec ) << endl;
    
    intVec = { -2, 0, -1 };
    cout << "DEBUG : kadane.getLargestProductContiguousArray() == 0 : " << kadane.getLargestProductContiguousArray( intVec ) << endl;
    
    intVec = { -2, 3, -4 };
    cout << "DEBUG : kadane.getLargestProductContiguousArray() == 24 : " << kadane.getLargestProductContiguousArray( intVec ) << endl;

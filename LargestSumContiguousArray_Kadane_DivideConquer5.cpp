...
//
class LargestSumKadane {
public:

    int getLargestSumContiguousArray( const vector< int >& intVec ) {
        if ( intVec.empty() ) return numeric_limits< int >::lowest();
        if ( intVec.size() == 1 ) return intVec[ 0 ];

        int curSum = intVec[ 0 ];
        int maxSum = intVec[ 0 ];

        for ( int i = 1; i < intVec.size(); i++ ) {
            curSum = max( curSum + intVec[ i ], intVec[ i ] );
...
//
class LargestSumDivideConquer {
private:

    const vector< int > * intVec = NULL;

    int findBestSubArray( int left, int right ) const {
        if ( left > right ) return numeric_limits< int >::lowest();
        ...
        int bestCombinedSum = bestLeftSum + this->intVec->at( mid ) + bestRightSum;
...
public:
    ...
    int getLargestSumContiguousArray( const vector< int >& intVec ) {
        this->intVec = &intVec;
        return findBestSubArray( 0, this->intVec->size() - 1 );
    }
...
int main( int argc, char ** argv ) {
    ...
    LargestSumKadane kadane;

    vector< int > intVec = { -2, 1, -3, 4, -1, 2, 1, -5, 4 };
    cout << "DEBUG : kadane.getLargestSumContiguousArray() == 6 : " << kadane.getLargestSumContiguousArray( intVec ) << endl;
    
    intVec = { -2, -3, 4, -1, -2, 1, 5, -3 };
    cout << "DEBUG : kadane.getLargestSumContiguousArray() == 7 : " << kadane.getLargestSumContiguousArray( intVec ) << endl;
    
    intVec = { -2, -3, -1, -5 };
    cout << "DEBUG : kadane.getLargestSumContiguousArray() == -1 : " << kadane.getLargestSumContiguousArray( intVec ) << endl;
    ...
    intVec = {};
    cout << "DEBUG : kadane.getLargestSumContiguousArray() == numeric_limits< int >::lowest() : " << kadane.getLargestSumContiguousArray( intVec ) << endl;

    cout << endl;
    
    LargestSumDivideConquer divideConquer;

    intVec = { -2, 1, -3, 4, -1, 2, 1, -5, 4 };
    cout << "DEBUG : divideConquer.getLargestSumContiguousArray() == 6 : " << divideConquer.getLargestSumContiguousArray( intVec ) << endl;
    
    intVec = { -2, -3, 4, -1, -2, 1, 5, -3 };
    cout << "DEBUG : divideConquer.getLargestSumContiguousArray() == 7 : " << divideConquer.getLargestSumContiguousArray( intVec ) << endl;
    
    intVec = { -2, -3, -1, -5 };
    cout << "DEBUG : divideConquer.getLargestSumContiguousArray() == -1 : " << divideConquer.getLargestSumContiguousArray( intVec ) << endl;
    ...
    intVec = {};
    cout << "DEBUG : divideConquer.getLargestSumContiguousArray() == numeric_limits< int >::lowest() : " << divideConquer.getLargestSumContiguousArray( intVec ) << endl;

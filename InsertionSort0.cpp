...
public:
    ...
    static void sort( vector< Value >& valuesVec ) {
        for ( int i = 1; i < valuesVec.size(); ++i ) {
            for ( int j = i; j > 0 && valuesVec[ j ] < valuesVec[ j - 1 ]; --j ) {
                swap( valuesVec[ j ], valuesVec[ j - 1 ] );
            }
        }
    }
...
int main( int argc, char ** argv ) {
    ...
    Insertion< int >::sort( intArr );
    cout << "DEBUG : Insertion< int >::sort() : " << endl;
